...
subtitle: "Version 1.1 of Combined Factor Analysis (No Chasing Data), duplicated records removed"
author:
    name: Liz Hare, PhD
...
**Note: this output contains numerous warnings from the factor analysis process.** 
This is because it is not straightforward to calculate the factor scores when using
ordered categorical items. It doesn't mean the overall models are bad.
...
preImputedCor <- lavaan::lavCor(ddItemsOrdered,
                                ...
                                missing = "pairwise",
                                cor.smooth = TRUE)
rownames(preImputedCor) <- colnames(preImputedCor)
...
Testing analyses from 11 - 18 factors, since parallel analysis suggested 16 and MAP 
suggested 
...
```
There are  items with low communality.


# Frequencies for Items with Low Loadings

```{r, lowLoadings, eval=TRUE, echo=FALSE}
source("freqLowLoad.R")
k2k <- kable(k2)
kable_styling(k2k, full_width = FALSE)
	

```

## Loadings < 0.32 for which factors?

``` {r, whichLowbyModel, eval=TRUE, echo=FALSE}

llfiles <- list.files(path = "data/",
pattern = "ll\\d\\d",
full.names = TRUE)

for (i in llfiles) {load(i)}

whichLL <- data.frame(c("TRAIN08", "AGG17", "FEAR22", "FEAR25",
"FEAR29", "FEAR30", "ATT39", "ATT40", "ATT42", "MISC50", "MISC57", 
"MISC58", "MISC68", "MISC70", "MISC71"))
colnames(whichLL) <- "item"

whichLL$Factors12 <- ifelse(whichLL$item %in% m1small12$itemNames,
                            "TooSmall", "")
whichLL$Factors13 <- ifelse(whichLL$item %in% m1small13$itemNames,
                            "TooSmall", "")
whichLL$Factors14 <- ifelse(whichLL$item %in% m1small14$itemNames,
                            "TooSmall", "")
whichLL$Factors15 <- ifelse(whichLL$item %in% m1small15$itemNames,
                            "TooSmall", "")
whichLL$Factors16 <- ifelse(whichLL$item %in% m1small16$itemNames,
                            "TooSmall", "")
whichLL$Factors17 <- ifelse(whichLL$item %in% m1small17$itemNames,
                            "TooSmall", "")
whichLL$Factors18 <- ifelse(whichLL$item %in% m1small18$itemNames,
                            "TooSmall", "")

whichLLK <- kable(whichLL)
kable_styling(whichLLK)
