...
``` {r, modelFit{{nfact}}, eval=TRUE}

fanal <- fa(postImputedCor,
      nfactors = {{nfact}},,
      n.obs = nrow(ddItems),
      rotate = "oblimin",
      fm = "wls")
...
- chi-square: `r fanal$STATISTIC`  
- degrees of freedom: `r fanal$dof`  
- P-value for chi-square = `r fanal$PVAL`  
...
``` {r, commun{{nfact}}, eval=TRUE}
comTab <- kable(data.frame(fanal$communality), digits = 2)
...
``` {r, finalLoadings{{nfact}}, eval=TRUE}
loadMat <- matrix(fanal$loadings, nrow = ncol(ddItems))
dimnames(loadMat) <- dimnames(fanal$loadings)
...
``` {r, loadwithDesc{{nfact}}, eval=TRUE}
