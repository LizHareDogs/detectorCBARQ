...
imputedFactors{{nfact}} <- imputationResults{{nfact}}$completeObs
save(imputedFactors{{nfact}}, file=paste0("data/imputedFactors", {{nfact}}, ".Rda"))
imputedChar{{nfact}} <- lapply(imputedFactors{{nfact}}, as.character)
...
preImputedCor{{nfact}} <- lavaan::lavCor(ddOrd{{nfact}},
                                ordered = names(ddOrd{{nfact}}),
                                missing = "pairwise",
                                cor.smooth = TRUE)
write.csv(round(preImputedCor{{nfact}}, 3), file=paste0("data/polychoricCorrelations", {{nfact}}, ".csv"), quote=FALSE, row.names=FALSE)
...
postImputedCor{{nfact}} <- lavCor(imputedNumericDF{{nfact}},
                        ordered = names(imputedNumericDF{{nfact}}),
                                 cor.smooth=TRUE)
write.csv(round(data.frame(postImputedCor{{nfact}}), 3), file="data/postImputedCor{{nfact}}.csv")
...
parallelOutput{{nfact}} <- fa.parallel(postImputedCor{{nfact}},
                              n.obs = nrow(imputedNumericDF{{nfact}}),
                                fa = "fa",
                                fm = "pa",
                              n.iter = 100)
...
fanal{{nfact}} <- fa(postImputedCor{{nfact}},
      nfactors = {{nfact}},
      n.obs = nrow(ddOrd{{nfact}}),
      rotate = "oblimin",
      fm = "pa",
      smooth = TRUE,
      cor = "poly")
...
``` {r, omega{{nfact}}, eval = TRUE, echo = FALSE}
omegas{{nfact}} <- omega(ddNum{{nfact}},
                         nfactors = {{nfact}},
                         fm = "pa",
##                       poly = TRUE,
                         rotate = "oblimin")
...
save(omegas{{nfact}}, file = paste0("data/omegas", {{nfact}}, ".Rda"))

```

The overall alpha for all items is 
`r omegas{{nfact}}$alpha`.  

The overall omega for all items is
`r omegas{{nfact}}$omega.tot`.  

Hierarchical omega is
`r omegas{{nfact}}$omega_h`.

``` {r, alphaOmegaTab{{nfact}}, eval = TRUE, echo = FALSE}
relTab <- omegas{{nfact}}$omega.group

#### Still need to calculate factor scores for Cronbach's alpha
